...
func newInitCmd() *cobra.Command {
	...
	var adapterFlag string // BUG-111: kept only for migration diagnostic
	var forceFlag bool
...
generating a gtms.config file, and setting up adapter support for the chosen
workflow preset.
...
Plain 'gtms init' in an empty repository scaffolds the 'manual' default preset
(the beginner-friendly path). In an already-initialised project it lists the
available presets and exits without mutation. Use '--presets' to list presets
without scaffolding, regardless of whether the repository is empty.

Examples:
  gtms init                          -- scaffold with the manual default preset (empty repo)
  ...
  gtms init --name "My Project"      -- specify project name (requires --preset)
...
re-run init. See ENH-154 for tracked future support.`,
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// BUG-111: Reject legacy --adapter flag with migration diagnostic.
			// Post-merge revision: drop the "Use: gtms init --preset <value>" line
			// because it echoes the rejected value verbatim and can suggest a
			// retired preset name (e.g. "minimal"). The "Available presets" line
			// is sufficient and always correct.
			if adapterFlag != "" {
				output.Errorf(
					"The --adapter flag has been replaced by --preset.",
					fmt.Sprintf("Available presets: %s", strings.Join(scaffold.ValidPresets(), ", ")),
				)
				return output.AsDisplayed(fmt.Errorf("--adapter flag removed; use --preset"))
			}
			...
			//   - plain `gtms init` / `--force` in existing project -> list + ENH-154 hint, no mutation.
			...
			if presetsFlag || (presetFlag == "" && !demoFlag && !guidanceOffFlag && !guidanceOnFlag) {
				...
				// Plain init / --force at the project root: list, hint
				// ENH-154, no mutation.
				if existingProject {
					fmt.Fprintf(os.Stderr, "This project is already initialized.\n\n")
					printPresetList(os.Stderr)
					fmt.Fprintf(os.Stderr, "\nTo add or switch presets in an existing project, see ENH-154.\n")
					return nil
				}
	...
	cmd.Flags().StringVar(&adapterFlag, "adapter", "", "")
	...
	cmd.Flags().BoolVar(&demoFlag, "demo", false, "Seed demo data for learning the GTMS pipeline")

	// BUG-111: Hide --adapter; it is kept only for the migration diagnostic.
	cmd.Flags().MarkHidden("adapter")
...
func formatInitOutput(root, name, repo, preset string, result *scaffold.Result, guidanceEnabled bool) {
	...
	fmt.Println("    gtms/cases/ (with guides)")
	...
	// Common adapter scripts
	fmt.Println("    gtms/adapters/create-skeleton.sh")
	fmt.Println("    gtms/adapters/manual-prime.sh")
	fmt.Println("    gtms/adapters/manual-execute.sh")

	// Preset-owned assets
	for _, f := range result.FilesCreated {
		if strings.HasPrefix(f, "gtms/adapters/") &&
			f != "gtms/adapters/create-skeleton.sh" &&
			f != "gtms/adapters/agent-skeleton.sh" &&
			f != "gtms/adapters/manual-prime.sh" &&
			f != "gtms/adapters/manual-execute.sh" {
			fmt.Printf("    %s\n", f)
		}
	}
...
func formatDemoOutput(root string, result *scaffold.DemoSeedResult, guidanceEnabled bool) {
	...
	// Build "what happened" for guidance block
	var whatLines []string
	for _, f := range result.FilesCreated {
		whatLines = append(whatLines, fmt.Sprintf("Created %s", f))
	}
	if result.ConfigModified {
		whatLines = append(whatLines, "Merged demo adapters into gtms.config")
	}
	whatHappened := strings.Join(whatLines, "\n")
...
func flatLayoutErrorMessage(presentDirs []string, goos string) (string, string) {
	...
	recipe.WriteString("GTMS v0.2+ uses gtms/cases/, gtms/automation/ etc.\n    To migrate:\n")
