...
This is the authoring reference for test cases in ` + "`gtms/cases/`" + `. It documents
...
// testCasesReadme is the README for the gtms/cases/ directory.
...
adapters:
  create:
    local-claude:
      mode: sync
      prompt-template: gtms/cases/prompts/create-standard.md    # ← prompt template
      guide-dir: gtms/cases/guides/                              # ← quality standards
...
GTMS streams stdout and writes each file to ` + "`gtms/cases/`" + ` as it completes. If no ` + "`<gtms-file>`" + ` tags are found, the output is captured as a summary but no test case files are created.
`
...
create: |
  ...
  Tip: edit gtms/adapters/create-skeleton.sh to customise the test case template
...
// agentSkeletonScript is the Tier 2 create adapter script for agent workflows.
// ENH-150: Scaffolded alongside the existing skeleton script but NOT registered
// in gtms.config. Dormant by design — CON-027's `gtms init <agent>` wires it
// into agent presets when ready. Identical to skeleton on day one; separate name
// for future agent-specific customisation.
const agentSkeletonScript = `#!/bin/sh
# Agent skeleton create adapter — writes a blank test case for agent authoring.
# Edit this script to customise the skeleton for your agent's conventions.
# NOTE: This script is scaffolded but not registered in gtms.config.
# To activate it, add an entry under adapters.create in gtms.config.
...
# Build frontmatter — include requirement only when --reference was passed
if [ -n "$GTMS_REFERENCE" ]; then
REQ_LINE="requirement: ${GTMS_REFERENCE}"
cat > "$OUTFILE" <<TCEOF
---
test_case_id: ${ID}
title: "${NAME_VALUE}"
...
adapter: agent-skeleton
mode: sync
status: complete
result: pass
artefact: ${OUTFILE}
summary: "Created agent skeleton test case ${ID}"
completed: $(date -u +%Y-%m-%dT%H:%M:%SZ)
REOF
fi

echo "Created agent skeleton test case: ${OUTFILE}"
` + ""

// skeletonCreateScript is the Tier 2 create adapter script shipped by gtms init.
// It produces a single markdown skeleton test case that the user fills in manually.
// Uses GTMS_OUTPUT_DIR, GTMS_TC_IDS, GTMS_REFERENCE, GTMS_RESULT_FILE, GTMS_TASK_ID.
const skeletonCreateScript = `#!/bin/sh
# Skeleton create adapter — writes a blank test case for manual authoring.
# Edit this script to customise the skeleton for your team's conventions.
...
result: pass
artefact: ${OUTFILE}
summary: "Created skeleton test case ${ID}"
completed: $(date -u +%Y-%m-%dT%H:%M:%SZ)
REOF
fi

echo "Created skeleton test case: ${OUTFILE}"
`
...
1. Choose a preset: ` + "`gtms init --adapter claude`" + ` or ` + "`gtms init --adapter github`" + `
...
const batsRunnerScript = `#!/bin/bash
...
# manual-prime.sh — Tier 2 sync adapter for manual test result preparation.
...
# manual-execute.sh — Tier 2 sync adapter for manual test result execution.
