...
func newResetCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Clear execute results from the pipeline dashboard",
		Long: `Clear execute results so the pipeline dashboard returns to a clean state.

  gtms reset                    — clear root-level test case results
  gtms reset bug-022            — clear results in gtms/cases/bug-022/
  gtms reset bug-022 -r         — clear results in bug-022/ and subdirectories
  gtms reset -r                 — clear all results across all folders
  gtms reset tc-a1b2c3d         — clear result for a single test case
